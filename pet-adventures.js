...
const ADVENTURE_REALMS = [
    {
        ...
        maxLuckyBonus: 1,
        element: 'earth',
        minFullness: 15,            // 出发所需最低饱食度（门槛，与消耗无关）
        minCleanliness: 0,          // 出发所需最低清洁度
...
const ADVENTURE_HELPERS = {
    ...
    calcLootCount: function(realm, bondValue) {
        ...
        var luckyBonus = Math.random() < ADVENTURE_CONFIG.luckyBonusChance
            ...
            : 0;
        return Math.max(1, base + bondBonus + luckyBonus);
    },
    ...
    // 28 级以上单次探险总收益上限（相对日均基础的倍数）
    TOTAL_REWARD_CAPS: {
        forest: { goldMul: 10, xpMul: 10, stars: 30 },
        creek:  { goldMul: 15, xpMul: 15, stars: 60 },
        ruins:  { goldMul: 20, xpMul: 20, stars: 100 },
        rift:   { goldMul: 25, xpMul: 25, stars: 120 },
        cosmos: { goldMul: 35, xpMul: 35, stars: 200 },
    },
    ...
    resolveLoot: function(rolledItems, dailyGoldBase, dailyXPBase, multipliers, gateMultiplier, event, currentLevel, realmId) {
        ...
        // 28 级以上追加单次探险总收益上限，防止高倍率宝物堆叠出现 jackpot
        if (currentLevel && currentLevel >= 28 && realmId) {
            var totalCap = self.TOTAL_REWARD_CAPS[realmId];
            if (totalCap) {
                var goldCap = Math.round(dailyGoldBase * totalCap.goldMul);
                if (totalGold > goldCap) {
                    var goldScale = goldCap / totalGold;
                    resolvedItems.forEach(function(item) { item.gold = Math.round(item.gold * goldScale); });
                    totalGold = goldCap;
                }
                var xpCap = Math.round(dailyXPBase * totalCap.xpMul);
                if (totalXP > xpCap) {
                    var xpScale = xpCap / totalXP;
                    resolvedItems.forEach(function(item) { item.xp = Math.round(item.xp * xpScale); });
                    totalXP = xpCap;
                }
                if (totalStars > totalCap.stars) {
                    var starsScale = totalCap.stars / totalStars;
                    resolvedItems.forEach(function(item) { item.stars = Math.round(item.stars * starsScale); });
                    bonusStars = Math.round(bonusStars * starsScale);
                    totalStars = totalCap.stars;
                }
            }
        }
